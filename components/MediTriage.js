...
export default MediTriageDemo;import React, { useState, useEffect } from 'react';
import { AlertCircle, Activity, Heart, Info, CheckCircle, AlertTriangle, Clock, Brain, ChevronRight, RefreshCw, FileText, BarChart3, MessageSquare } from 'lucide-react';

const MCNZTriageCompleteDemo = () => {
  const [complaintText, setComplaintText] = useState('');
  const [result, setResult] = useState(null);
  const [loading, setLoading] = useState(false);
  const [showExamples, setShowExamples] = useState(true);
  ...
  const complexExamples = [
    {
      id: 1,
      text: "Dr Smith was late for the appointment. I was in pain and had to wait for 15 mins before being seen by him. This has happened twice now.",
      category: "CONDUCT",
      complexity: "Pattern Recognition",
      expectedOutcome: "Identifies repeated behavior pattern affecting patient care"
    },
    ...
    {
      id: 4,
      text: "My GP prescribed medication without examining me properly. When I questioned this, she became defensive and dismissed my concerns.",
      category: "DUAL_ISSUE",
      complexity: "Multiple Issues",
      expectedOutcome: "Both competence (inadequate examination) and conduct (defensive behavior) issues"
    },
  ...
  const simulateAIClassification = (text) => {
    ...
    // Sentiment analysis
    const negativeWords = ['worse', 'pain', 'worried', 'defensive', 'dismissed', 'shaking', 'impaired'];
    const positiveWords = ['helped', 'correct', 'thorough', 'good', 'proper'];
        ...
        <div className="bg-white shadow-lg rounded-xl p-6 mb-6">
          <div className="flex items-center justify-between flex-wrap gap-4">
            <div>
              <h1 className="text-3xl font-bold text-gray-900 flex items-center gap-3">
                <Brain className="w-8 h-8 text-blue-600" />
                MCNZ AI Triage System
              </h1>
              <p className="text-gray-600 mt-2">Advanced Context-Aware Complaint Classification</p>
            </div>
            <div className="flex items-center gap-4">
              <div className="text-center">
                <div className="text-2xl font-bold text-blue-600">{processedComplaints.length}</div>
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            <div className="bg-white rounded-xl shadow-lg overflow-hidden">
              ...
              <div className="p-6">
                {activeTab === 'input' && (
                  <div>
                    <h2 className="text-xl font-semibold text-gray-900 mb-4">Enter Complaint Details</h2>
...
export default MCNZTriageCompleteDemo;